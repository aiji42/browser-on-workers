...
- **フォントはシステムから取れない。** Workers にフォントが 1 つも無いので、TTF を自分で持ち込んで Parley に登録するしかない。woff2 は Brotli で圧縮されていて Workers 側でほどけないので、TrueType のまま置く。バンドルには埋め込まず Static Assets から読む (Kitesurf の PageRenderer と同じ)。日本語 1 ウェイトで 2.2 MB あるので、スクリプトサイズの上限に効かせない方が楽で、代わりに cold start で 20〜35 ms かかる
