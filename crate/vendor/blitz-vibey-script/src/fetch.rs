...
impl ScriptFetcher for DefaultScriptFetcher {
    fn fetch(&self, url: &Url) -> Result<String, FetchError> {
        match url.scheme() {
            // **local change**: `Url::to_file_path` and `std::fs` do not exist
            // on wasm, where there is no filesystem to read a script from
            // anyway. Embedders there install their own `ScriptFetcher`.
