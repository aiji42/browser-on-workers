...
<ul>
  ...
  <li>ページの JavaScript には実行上限があります。<code>while (true)</code> はループ 50 万回で止まります。Boa には実行を中断する仕組みが無いので、上限を自分で設定しています</li>
  <li>展開すると大きくなりすぎる画像は飛ばします。isolate のメモリに載らないためです</li>
  <li>ページの JavaScript が描画を壊したときは、JS を切って描き直します。そのときスクリプトの効果は入りません</li>
  <li>Web Workers と WebAssembly をページに出していません。使える機能を見て自分で「非対応」と出すサイトは、そのまま非対応の表示になります</li>
...
<footer>
  発表記事に載っている「Chromium より CPU が 3.1 倍少ない」という数字を外から測ろうとして、測る手段が無いと分かったので、それなら同じ構成で作ってみようと始めたものです。中身は見ていないので、同じクレートでも同じものにはなっていません。組んでみて分かったことは <a href="https://aiji42.dev/blog/kitesurf-two-js-engines/">Kitesurf を自作してわかった、JS エンジンを 2 つ積む理由</a> に書きました。
</footer>
